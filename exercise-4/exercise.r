...
movie.data <- fromJSON(request.content)
...
Director <- function(movie.info) {
  flattened <- flatten(movie.info)
  movie.dir <- paste0(flattened[1,]$Title, ' was directed by ', flattened[1,]$Director)
  return (movie.dir)
}
...
movie.list <- c("10 Things I Hate About You", "Kingsman: The Secret Service", "Dope")

# Write a function to make your API request and process the data
Request <- function(movie) {
  my.query <- list(t = movie, r = 'json')
  # Make a variable called request that is a string of a request URL made up of the base URL
  # and the parameters string
  request <- GET(base, query = my.query)
  request.content <- content(request, 'text')
  data <- as.data.frame(fromJSON(request.content))
  data <- flatten(data)
  return (data)
}
